...
export default function ManualPage() {
  ...
  return (
    <div className="manual-page">
      <div className="manual-layout">
        <div className="manual-left">
          <header className="manual-header-left">
            <h2>Manual Entry (Map-style)</h2>
            <p>Use the form below to select start and destination.</p>
          </header>

          <div className="manual-left-content">
            <SearchBar onSearch={handleSearch} onLocationSelect={setSelectedLocation} startValue={startLocation} endValue={endLocation} showNaturalInput={false} />
        ...
        <aside className="manual-output" role="region" aria-label="Navigation output panel">
          {navigationData ? (
            <>
      ...
      {selectedLocation && (
        <LocationDetails location={selectedLocation} onClose={() => setSelectedLocation(null)} onNavigate={() => {}} />
      )}
    </div>
  );
}
